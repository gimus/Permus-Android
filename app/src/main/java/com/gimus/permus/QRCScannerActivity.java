...
public class QRCScannerActivity extends AppCompatActivity
    ...
    public static void getQRCode(Activity caller) {
        Intent startNewActivity = new Intent(caller, QRCScannerActivity.class);
        startNewActivity.putExtra("request", SCAN_QRCODE);
        caller.startActivityForResult(startNewActivity, SCAN_QRCODE);
    }

    @Override protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        setContentView(R.layout.activity_qrcscanner);
    ...
    @Override public void onQRCodeRead(String text, PointF[] points) {
        ...
        switch (requestCode) {
            case SCAN_QRCODE:
                setResult(Activity.RESULT_OK, resultIntent.putExtra("QRCode",QRCode));
                finish();
                break;
        }
    }
        ...
        if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.CAMERA)) {
            Snackbar.make(mainLayout, "Camera access is required to display the camera preview", Snackbar.LENGTH_INDEFINITE).setAction("OK", new View.OnClickListener() {
                @Override public void onClick(View view) {
                    ActivityCompat.requestPermissions(QRCScannerActivity.this, new String[] { Manifest.permission.CAMERA }, MY_PERMISSION_REQUEST_CAMERA);
                }
            }).show();
        } else {
